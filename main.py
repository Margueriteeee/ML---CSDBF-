...
from generate_pic import generate_png, generate_png_low
from mark import mark_errors
from mark import mark_errors_low

device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")

global Dataset

Dataset = "FM"  # RV, FM, USA, YC, SANTA, BAY

# RV ：random，25，0.0336
# FM ：random，10，0.2095
# USA ：random，15，0.0977

# YC ：random，15，0.005
# SANTA ：low，450，0.005
# BAY ：low，80，0.005

sampling_mode = "random"   # random ，low
scale = 10
ITER, lr, num_epochs, sampling_rate = 4, 1e-4, 1000, 0.2095
...
for index_iter in range(ITER):
    ...
    # 模型训练
    train_losses, train_OAs, val_losses, val_OAs=train.train(net, lr, num_epochs, net_input_before, net_input_after, net_input_concat, train_gt, train_onehot, train_mask, val_gt, val_onehot, val_mask)
    ...
    if(dataset_name == "BAY"):
        # 取出标记后的数组
        CD_result = generate_png_low(classification_map, gt, "results\\" + dataset_name + str(test_OA))
    else:
        CD_result = generate_png(classification_map, "results\\" + dataset_name + str(test_OA))

    # generate_png(classification_map, "results\\" + dataset_name + str(test_OA))
    
    # 调用标记函数
    detection_result = classification_map.cpu().numpy()
    detection_result = np.where(detection_result > 1, 0, 1)  # 转换为0和1表示的数组
    ground_truth = gt  # 假设gt是真值变化图
    if(dataset_name == "BAY"):
        mark_errors_low(CD_result, ground_truth, "results\\" + dataset_name + str(test_OA))
    else:
        mark_errors(detection_result, ground_truth, "results\\" + dataset_name + str(test_OA))
